...
exports.handlerSubpackages = function handlerSubpackages(appJson) {
  appJson.subpackages = appJson.subpackages || []
  const list = appJson.subpackages.map(item => {
    return {
      name: item.root,
      value: item.root,
    }
  ...
  list.unshift({
    name: 'pages(the default package)',
    value: 'pages',
  })
  list.push({
    name: ' you can input the new package name in next step',
    value: 'newPackage',
  })
  return list
}

exports.getAppJson = function getAppJson() {
  const cwd = process.cwd()
  if (fse.existsSync(`${cwd}/app.json`)) {
    return require(`${cwd}/app.json`)
  } else {
    console.log(
      chalk.yellow(
        'there is no app.json,\n' + ' please check the miniProgram project'
      )
    )
    // process.exit(-1)
  }
...
exports.isMiniProgramProject = function isMiniProgramProject() {
  const jsonPath = path.join(process.cwd(), 'app.json')
  return fse.existsSync(jsonPath)
}
