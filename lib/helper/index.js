...
exports.getPkgBoilerplates = function getPkgBoilerplates() {
  if (process.env.NODE_ENV === 'test') {
    return ['default', 'react', 'typescript', 'react-typescript', 'custom']
  }
...
exports.getTestConfig = function getTestConfig(type) {
  const pkgInfos = {
    ...
    'jest-react': require('../../test-config/jest-react/package.json'),
  }
  const types = Object.keys(pkgInfos)
  if (types.includes(type)) {
    return pkgInfos[type]
  }
  throw new TypeError(
    `Invalid type: ${type}\nAvailable type are: ${types.join('、')}`
  )
}
