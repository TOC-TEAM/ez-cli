...
const { initGitRepo, getToolConfig } = require('../../helper')
...
module.exports = async function createLibrary(answers) {
  ...
  const files = await globby(fromDir, {
    dot: true,
  })
  ...
  ora.promise(promise, `Copying ${template} template to ${toDir}`)
  await promise

  // The order of initGit and initPackage is important:
  // husky bind git hooks need git first
  if (git) {
    const promise = initGitRepo({ cwd: toDir })
...
async function copyTemplateFile(opts) {
  ...
  if (base === 'package.json') {
    pkgObj = require(file)
    pkgObj = _.merge(pkgObj, getToolConfig(answers.test))
